...
const VariableFields = forwardRef<VariableFieldsHandle, VariableFieldsProps>(({ templateVariables, templateId, tokenResponse, selections, disabled, onVariablesChange }, ref) => {
  ...
  if (loading) {
    return <Loader loadingText="Loading variables" />;
  }
  ...
  return (
    <div className="flex flex-col">
      {allVars.map((v, index) => {
        ...
        return (
          <VariableCard key={id} varIndex={index} label={label} hint={hint} required={Boolean(v.required)} readOnly={!editable} footerLeft={footerLeft} footerRight={footerRight}>
            <VariableControl
              type={type}
              options={options}
              isEditable={editable}
